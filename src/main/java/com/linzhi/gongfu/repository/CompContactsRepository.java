...
public interface CompContactsRepository extends CrudRepository<CompContacts, CompContactsId>, QuerydslPredicateExecutor<CompContacts> {
    ...
    List<CompContacts> findCompContactsByCompContactsId_AddrCodeAndCompContactsId_DcCompIdAndCompContactsId_OperatorCodeOrderByContCompName(String addrCode, String dcCompId, String operator) ;
    ...
    @Query(value="select  right(('000'+cast((cast(max(right(code,3)) as int)+1) as varchar)),3) " +
        ...
        nativeQuery = true)
    String findMaxCode(String dcCompId,String addrCode,String operator);
    @Modifying
