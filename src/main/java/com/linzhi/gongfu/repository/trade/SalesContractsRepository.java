...
public interface SalesContractsRepository
    ...
    @Query(value = "select * from (select" +
        ...
        ",pv.order_code as customerContractNo ,b.buyer_comp as buyerCompCode  ,cb.chi_short  as buyerCompName," +
        ...
        "left join purchase_contract_rev pv on pv.id = p.id and pv.revision in (select max(revision) from purchase_contract_rev  where id = pv.id)\n "+
        " where b.created_by_comp=?1 and b.created_by=?2 and b.state =?3   group by  b.id,b.code,d.revision,d.order_code,o.code,o.name, b.created_at,b.state,p.id,d.buyer_order_code,b.buyer_comp ,cb.chi_short,d.total_price_vat,d.confirm_total_price_vat,pv.order_code) c order  by c.createdAt ,cast(RIGHT(c.code,3) as int ) desc \n", nativeQuery = true)
    ...
    @Query(value = "select * from (select " +
        ...
        "left join purchase_contract_base p on b.paired_code=p.paired_code " +
        "left join purchase_contract_rev pv on pv.id = p.id and pv.revision in (select max(revision) from purchase_contract_rev  where id = pv.id)\n "+
        " where b.created_by_comp=?1  and b.state =?2   group by  b.id,b.code,d.revision,d.order_code,o.code,o.name, b.created_at,b.state,p.id,d.buyer_order_code,b.buyer_comp ,cb.chi_short,d.total_price_vat,d.confirm_total_price_vat,pv.order_code) c order  by c.createdAt ,cast(RIGHT(c.code,3) as int ) desc \n", nativeQuery = true)
